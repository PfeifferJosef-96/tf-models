...
def main(_):
  ...
  if not tf.io.gfile.exists(FLAGS.original_image):
    print('\nCannot find --original_image.\n')
    return

  if not tf.io.gfile.exists(FLAGS.compared_image):
    print('\nCannot find --compared_image.\n')
    return

  with tf.compat.v1.gfile.FastGFile(FLAGS.original_image) as image_file:
    img1_str = image_file.read('rb')
  with tf.compat.v1.gfile.FastGFile(FLAGS.compared_image) as image_file:
    img2_str = image_file.read('rb')

  input_img = tf.compat.v1.placeholder(tf.string)
