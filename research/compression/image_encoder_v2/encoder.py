...
tf.compat.v1.flags.DEFINE_string('input_image', None, 'Location of input image. We rely '
                       'on tf.image to decode the image, so only PNG and JPEG '
                       'formats are currently supported.')
tf.compat.v1.flags.DEFINE_integer('iteration', 15, 'Quality level for encoding image. '
                        'Must be between 0 and 15 inclusive.')
tf.compat.v1.flags.DEFINE_string('output_codes', None, 'File to save output encoding.')
tf.compat.v1.flags.DEFINE_string('model', None, 'Location of compression model.')
...
def load_ppm(filename):
    ...
    pad0 = (np.ceil(im.shape[0]/32) * 32).astype(int)
    pad1 = (np.ceil(im.shape[1]/32) * 32).astype(int)

    im_n = np.zeros((pad0,pad1, im.shape[-1]))
    im_n[:im.shape[0], :im.shape[1], :] = im

    image = tf.convert_to_tensor(im_n)
...
def main(_):
  ...
  with tf.compat.v1.gfile.FastGFile(FLAGS.input_image, 'rb') as input_image:
  ...
  with tf.Graph().as_default() as graph:
    ...
    if ext == '.png':
      ...
      decoded_image = load_ppm(FLAGS.input_image)#tf.expand_dims(decoded_image, 0)
    else:
  ...
  with tf.compat.v1.gfile.FastGFile(FLAGS.output_codes, 'w') as code_file:
