...
tf.compat.v1.flags.DEFINE_string('input_codes', None, 'Location of binary code file.')
tf.compat.v1.flags.DEFINE_integer('iteration', -1, 'The max quality level of '
                        'the images to output. Use -1 to infer from loaded '
                        ' codes.')
tf.compat.v1.flags.DEFINE_string('output_directory', None, 'Directory to save decoded '
                       'images.')
...
def main(_):
  ...
  #  tf.io.gfile.mkdir(FLAGS.output_directory)

  if not tf.io.gfile.exists(FLAGS.input_codes):
    print('\nInput codes not found.\n')
    return

  contents = ''
  with tf.compat.v1.gfile.FastGFile(FLAGS.input_codes, 'rb') as code_file:
  ...
  with tf.compat.v1.Session(graph=graph) as sess:
    ...
    for index, result in enumerate(results):
      ...
      with tf.compat.v1.gfile.FastGFile(FLAGS.output_directory,
                              'w') as output_image:
